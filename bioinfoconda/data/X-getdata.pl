...
# TODO: add progress bar to fix_index.
...
my $index_file_path = "$ENV{'BIOINFO_ROOT'}/bioinfoconda/log/data_index.log";

# Create index file if it does not exist
my ($indexfile, $indexdir) = fileparse($index_file_path);
if (!-d $indexdir)
{
	make_path($indexdir) or die("Failed to create path: $indexdir.\n");
}
if (!-f $index_file_path)
{
	open(my $FH, ">>", $index_file_path) or die("Can't open $index_file_path.\n");
	close($FH);
}
...
sub update_index
{
    ...
    open(INDEXFILE, ">>", $index_file_path)
	    or die "ERROR: cannot open $index_file_path.";
